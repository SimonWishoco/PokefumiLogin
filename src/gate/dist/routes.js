...
const getConnectedUsersList = (sessions) => {
    var connected_users = [];
...
const register = (app) => {
            ...
            if (Object.keys(results).length !== 0) {
                ...
                if (chosenPlayer === user) {
                    ...
                    if (connectedPlayers.includes(chosenPlayer)) {
                        var userOwnsDeck = userDecks.some((item) => (item.deck_id === chosenDeck));
    ...
    app.get('/ongoing_match', (req, res) => {
        ...
        if (token !== undefined) {
            ...
            var results = JSON.parse(request('GET', url).getBody());
            if (Object.keys(results).length !== 0) {
                var ongoingMatch = results[results.length - 1];
    ...
    app.post('/match/:card', (req, res) => {
        ...
        if (token !== undefined) {
            ...
            if (Object.keys(result).length !== 0) {
                var ongoingMatch = result[result.length - 1];
                var url = "http://0.0.0.0:" + matchesPort + "/match/development/" + ongoingMatch['match_id'];
